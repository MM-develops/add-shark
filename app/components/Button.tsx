...
export const Button = (props: { type?: string; className?: string; children?: any; onClick?: React.MouseEventHandler<HTMLElement> }) => {
    const type = props.type ? props.type : 'primary';
    return <button className={`${styles.button} ${props.className} ${styles[type]}`} onClick={props.onClick}>{props.children}</button>
}
